...
int main(int argc, char* argv[]) {
    ...
    std::ofstream data_set("Nov19Data/data.csv", std::ios_base::app);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		...
		plate_impact_state = true;
           ...
           if (t >= particle_moving  && plate_impact_state == true) {
            ...
            // Commented this out to see if the foot will move or not
	    ...
	    // This applies the torque as the system determines the force is
	    //rigid_plate->Accumulate_force(ChVector<>(plate_force[0], plate_force[1], plate_force[2]), plate_pos, false);
            //rigid_plate->Accumulate_torque(ChVector<>(plate_force[3], plate_force[4], plate_force[5]), false); 
	    //rigid_plate->Accumulate_force(ChVector<>(force_x, force_y, force_z), plate_pos, false);
	    //rigid_plate->Accumulate_force(ChVector<>(torque_x, torque_y, torque_z), plate_pos, false);
		...
		// Gamma, Beta, depth, position_x, position_z, velocity x, velocity y, GRF x, GRF Z
                double depth = gran_sys.get_max_z() - plate_pos[2];  // plate's Z position - gran_sys.get_max_z();
		...
		// For "natural" simulations where we let the DEM control everything
                double gamma_now = (Q_to_Euler123(rigid_plate->GetRot()))[1];  
                double beta_now = atan2(z_dt, x_dt);
		...
		data_set << t << ", " << gamma_now << ", " << beta_now << ", " << depth << ", " << position_x << ", " << position_y << ", " << position_z << ", " << x_dt << ", " << y_dt << ", " << z_dt << ", " << plate_ang_vel[0] << ", " << plate_ang_vel[1] << ", " << plate_ang_vel[2] << ", " << torque_x << ", " << torque_y << ", " << torque_z << ", " << grf_x << ", " << grf_y << ", " << grf_z << "\n";

		//if (position_z < ____) {
			// check for anomalies with this parameter 
		//}
            }
